...
BlpHandler::BlpHandler(ILcontext* context) :
	context(context)
{
...
ILboolean BlpHandler::isValid(ILconst_string FileName)
{
	ILHANDLE	BlpFile;
...
ILboolean BlpHandler::isValidF(ILHANDLE File)
{
	ILuint		FirstPos;
	ILboolean	bRet;
...
ILboolean BlpHandler::isValidL(const void *Lump, ILuint Size)
{
...
ILboolean BlpHandler::isValidInternal()
{
	BLP2HEAD Header;
...
//! Reads a BLP file
ILboolean BlpHandler::load(ILconst_string FileName)
{
	ILHANDLE	BlpFile;
	ILboolean	bBlp = IL_FALSE;
...
ILboolean BlpHandler::loadF(ILHANDLE File)
{
	ILuint		FirstPos;
	ILboolean	bRet;
...
ILboolean BlpHandler::loadL(const void *Lump, ILuint Size)
{
	iSetInputLump(context, Lump, Size);
	return loadInternal();
}
...
ILboolean BlpHandler::loadInternal()
{
	BLP2HEAD	Header;
	ILubyte		*CompData;
	ILimage		*Image;
